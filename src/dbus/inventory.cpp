...
static constexpr auto INVENTORY_BUS_NAME = "xyz.openbmc_project.Inventory.Manager";
...
static constexpr auto INVENTORY_ITEM_IFACE = "xyz.openbmc_project.Inventory.Item";
...
static constexpr auto INVENTORY_DECORATOR_I2CDEVICE_IFACE = "xyz.openbmc_project.Inventory.Decorator.I2CDevice";
...
static constexpr auto INVENTORY_MANAGER_IFACE = "xyz.openbmc_project.Inventory.Manager";
static constexpr auto INVENTORY_MANAGER_OBJECT = "/xyz/openbmc_project/inventory";
...
using PropertyType = std::variant<bool, size_t, int64_t, std::string, std::vector<uint8_t>>;
        ...
        { drive.getInventoryPath(), {
                { INVENTORY_IPZVPD_VINI_IFACE, {
                        ...
                        { "SN", std::vector<uint8_t>({
                                static_cast<uint8_t>(drive.getBackplane().getIndex()),
...
{
    std::string path = std::string("/xyz/openbmc_project/inventory") + drive.getInventoryPath();

    auto call = dbus.new_method_call(INVENTORY_BUS_NAME, path.c_str(), DBUS_PROPERTY_IFACE, "Set");
